...
- 当前实现中，`billing_daily_usage_metrics`、`billing_daily_ledger_summary` 已由 `src/api/flaskr/service/billing/models.py` 和 `src/api/migrations/versions/d4e5f6a7b8c9_add_billing_daily_aggregate_tables.py` 落地，先提供 schema、唯一索引和契约测试，聚合任务与 rebuild 入口留在后续条目实现

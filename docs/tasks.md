...
- [x] 实现 `billing_renewal_events` 的入队、抢占和幂等执行。
- [x] 实现 webhook 补偿同步和失败续费重试。
- [x] 保留 Flask CLI 作为 backfill / rebuild / manual replay 入口，并与 Celery 任务分工清晰。
...
- [x] 增加结算、余额计算、积分消耗顺序和三场景扣分测试。
- [x] 增加多个学生并发学习同一 creator 课程时，Celery 串行扣减仍然准确的测试。
- [x] 增加多 bucket 拆分扣减、bucket 过期和 `credit_wallets`/`credit_wallet_buckets`/`credit_ledger_entries` 一致性测试。
...
- [x] 实现创作者维度的权益快照和运行时解析逻辑。
- [x] 实现 `max_concurrency`、priority class 和 runtime admission enforcement。
- [x] 实现自定义域名绑定、校验、停用和 host 解析逻辑。
- [x] 扩展 runtime config，返回 entitlement、branding 和 domain 结果。
...
- [x] 实现 admin 侧域名审核、权益查看和报表页面。

### 测试与上线扩展

- [x] 增加权益快照、自定义域名和 host 解析测试。
- [x] 增加日报表、rebuild 和报表对账测试。
- [x] 增加 v1 升级到 v1.1 的迁移和回填验证。

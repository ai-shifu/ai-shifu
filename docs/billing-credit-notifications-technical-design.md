...
1. Celery 定时任务 `billing.scan_low_balance_notifications` 复用现有钱包和 billing overview 低余额判断。
2. 对命中的创作者和阈值生成 `dedupe_key = low_balance:{creator_bid}:{threshold}:{date}`。
3. 成功创建 `pending` 记录后 enqueue 发送任务。
4. v1 可以保留旧 `billing.send_low_balance_alert` 任务名作为兼容入口，内部委托给新扫描逻辑。

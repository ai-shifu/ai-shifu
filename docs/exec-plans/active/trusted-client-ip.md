...
All backend IP consumers now use one trust-aware result. Direct callers cannot change their address with a forwarding header, while explicitly trusted nginx/CDN chains resolve the first untrusted hop from the right. The production Compose API remains private and the development API remains loopback-only. Deployments must configure their exact proxy CIDRs before relying on per-client IP thresholds; the safe empty default intentionally refuses to guess a Docker or private network.

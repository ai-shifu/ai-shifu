...
- [x] 2026-09-24: Pinned stable `markdown-flow-ui@0.2.27` in Cook Web and the Arena, kept Spanish content language while routing embedded controls to English, and verified both installed packages, frontend tests and build, and Arena browser smoke.
...
- Its content components derive the HTML `lang` from the control locale unless supplied separately, so Spanish content must explicitly retain `lang=es-ES` during the English control fallback.
...
- 2026-09-24: The user then chose the existing stable `markdown-flow-ui@0.2.27` for this PR. Keep Spanish application and content localization, with English embedded controls as a documented temporary limitation; do not publish the component as part of this change.

## Outcomes & Retrospective

All 58 locale files and the application wiring are implemented. AI-Shifu PR #2942 is open for review. The earlier development package demonstrated native Spanish component text in a browser; this PR now uses stable `0.2.27`, which keeps Spanish content language but displays embedded component controls in English. A future stable Spanish-capable MarkdownFlow release can remove that fallback.
...
4. Pin the existing stable MarkdownFlow release and map Spanish to its English control locale while retaining Spanish content language; verify the installed package behavior in this product.

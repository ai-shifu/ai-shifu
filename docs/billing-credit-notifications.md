...
- 低余额阈值，例如余额为 `0`、低于固定积分数、低于百分比，或预计 N 天内耗尽。
...
v1 可先用 billing 动态配置承载运营策略，例如 `BILL_CREDIT_NOTIFICATION_SMS_CONFIG` JSON。若后续需要多活动、多模板版本、审批流和操作审计，应演进为独立的 notification policy 与 notification record 表。

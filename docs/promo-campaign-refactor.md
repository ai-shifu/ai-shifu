...
- `promo_promos`：促销活动/方案定义表（替代现有 `active`）
- `promo_redemptions`：促销方案订单级核销/应用台账（每次下单生成，替代现有 `active_user_record`）
...
### 1) `promo_promos`（促销活动/方案）

| 字段 | 类型 | 索引 | 默认值 | 说明 |
|---|---|---:|---|---|
| `id` | BIGINT | PK |  | 自增主键 |
| `promo_bid` | VARCHAR(36) | ✅ | `''` | Promotion 业务键 |
| `shifu_bid` | VARCHAR(36) | ✅ | `''` | 作用对象（当前等价于 `active_course` 的实际用法） |
| `name` | VARCHAR(255) |  | `''` | 名称 |
| `description` | TEXT |  | `''` | 描述 |
| `apply_type` | SMALLINT |  | `2101` | 2101=auto, 2102=event, 2103=manual |
...
建议索引：
- 单列索引：`promo_bid`、`shifu_bid`、`status`、`start_at`、`end_at`、`deleted`
- 可选复合索引：`(shifu_bid, status, start_at, end_at)`（用于快速查找可用促销）

不创建 DB 外键约束（遵循现有规范）。

### 2) `promo_redemptions`（订单级核销/应用台账）

| 字段 | 类型 | 索引 | 默认值 | 说明 |
|---|---|---:|---|---|
| `id` | BIGINT | PK |  | 自增主键 |
| `redemption_bid` | VARCHAR(36) | ✅ | `''` | Redemption 业务键（替代 `record_id`） |
| `promo_bid` | VARCHAR(36) | ✅ | `''` | 关联 `promo_promos.promo_bid` |
| `order_bid` | VARCHAR(36) | ✅ | `''` | 订单业务键 |
| `user_bid` | VARCHAR(36) | ✅ | `''` | 用户业务键 |
| `shifu_bid` | VARCHAR(36) | ✅ | `''` | Shifu 业务键 |
| `promo_name` | VARCHAR(255) |  | `''` | 名称快照（便于后台展示） |
...
建议索引：
- 单列索引：`redemption_bid`、`promo_bid`、`order_bid`、`user_bid`、`shifu_bid`、`status`、`deleted`
- 可选复合索引：
  - `(order_bid, deleted)`：订单详情/后台列表常用
  - `(promo_bid, user_bid, status, deleted)`：避免重复应用/便于统计
- 可选唯一约束（按业务是否允许同订单重复应用同 promo 决定）：`UNIQUE(order_bid, promo_bid, deleted)`
...
| 旧字段 | 新字段 | 说明 |
|---|---|---|
| `active_id` | `promo_bid` | 建议直接复用原值，避免额外映射 |
| `active_name` | `name` |  |
| `active_desc` | `description` |  |
| `active_join_type` | `apply_type` |  |
...
### `active_user_record` → `promo_redemptions`

| 旧字段 | 新字段 | 说明 |
|---|---|---|
| `record_id` | `redemption_bid` | 建议直接复用原值 |
| `active_id` | `promo_bid` |  |
| `active_name` | `promo_name` | 快照字段 |
...
1. 下单链路（当前在 `src/api/flaskr/service/order/funs.py`）：
   - 用 `promo_promos` 替代 `Active` 查询可用促销
   - 用 `promo_redemptions` 替代 `ActiveUserRecord` 写入/回查核销/应用记录
...
### Phase 1：新增表（不影响线上）
- Alembic 创建（或最终具备）`promo_promos`、`promo_redemptions`（不删旧表）。
- 新增 SQLAlchemy models（先不替换业务读写）。

### Phase 2：数据回填
- 通过一次性脚本或迁移脚本将旧表数据回填到新表：
  - `promo_bid`/`redemption_bid` 直接复用旧表 UUID 值，避免额外映射表。
...
2. 是否允许同一订单重复应用同一个 `promo_bid`（决定是否加 UNIQUE 约束）。

...
async def run_load_test():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(CONCURRENT_USERS):
            task = asyncio.create_task(user_behavior(session, f"user_{i}", run_stats, reset_stats))
            tasks.append(task)
    ...
    logger.info(f"Run Script API Stats: {json.dumps(run_stats.get_summary(), indent=2)}")
    logger.info(f"Reset Study Progress API Stats: {json.dumps(reset_stats.get_summary(), indent=2)}")
    
    if run_stats.deadlocks > 0 or reset_stats.deadlocks > 0:
        logger.error(f"Detected {run_stats.deadlocks + reset_stats.deadlocks} deadlocks during testing")
    
    if run_stats.timeouts > 0 or reset_stats.timeouts > 0:
        logger.error(f"Detected {run_stats.timeouts + reset_stats.timeouts} timeouts during testing")

...
class DatabaseMonitor:
    ...
    def stop(self):
        ...
        logger.info(f"Detected {self.lock_count} locks and {self.timeout_count} timeouts")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "DEADLOCK" in output:
                    ...
                    deadlock_section = re.search(r'LATEST DETECTED DEADLOCK(.*?)TRANSACTIONS', output, re.DOTALL)
                    if deadlock_section:
            ...
            if result.returncode == 0:
                output = result.stdout
                long_queries = [line for line in output.split('\n') if 'Time' in line and re.search(r'Time:\s+[3-9][0-9]+', line)]
                if long_queries:
                    logger.warning(f"Long running queries detected: {len(long_queries)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--duration", type=int, default=0, help="Total monitoring duration in seconds (0 for indefinite)")

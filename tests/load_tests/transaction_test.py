    ...
    try:
        if lock.acquire(blocking=True):
            try:
                ...
                with session.begin_nested():
                    record = session.query(TestRecord).filter(TestRecord.id == record_id).first()
                    if record:
                        record.status = 1
                        record.data = "Updated data"
                    session.commit()
...
def run_concurrency_test(db_uri, redis_host, redis_port, redis_db=0, redis_password=None):
    ...
    def test_unoptimized(user_id):
        ...
        try:
            for i in range(num_requests_per_user):
                result = run_without_optimization(session, redis_client, f"{user_id}_{i}")
                if result:
                    success_count += 1
                else:
                    failure_count += 1
                    if "deadlock" in str(session.execute("SHOW ENGINE INNODB STATUS").fetchall()).lower():
                        deadlock_count += 1
        finally:
            session.close()
    ...
    improvement = {
        "success_rate_improvement": (optimized_results["success_count"] / (num_users * num_requests_per_user)) - 
                                   (unoptimized_results["success_count"] / (num_users * num_requests_per_user)),
        "deadlock_reduction": unoptimized_results["deadlock_count"] - optimized_results["deadlock_count"],

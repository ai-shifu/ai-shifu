...
# Edit .env: configure an LLM provider API key and set LLM_TIER_FAST_MODEL
# to a text model ID served by that provider. Fast has no default model.
...
- `LLM_TIER_FAST_MODEL` must name a configured text model; leaving it empty prevents API startup. `LLM_TIER_BALANCED_MODEL` and `LLM_TIER_ULTIMATE_MODEL` are optional, have no defaults, and leave those choices unavailable when empty.
- For an existing installation, follow [Upgrading to model tiers](INSTALL_MANUAL.md#upgrading-to-model-tiers) before starting the new images. This applies to latest, pinned-release and development Compose modes.
...
# Edit .env: both a provider key and a Fast model mapping are required:
# - OPENAI_API_KEY / ERNIE_API_KEY / GLM_API_KEY / ...
# - LLM_TIER_FAST_MODEL: a configured text model ID (no default)
...
cp .env.example.full .env
# Edit .env: set your LLM API key(s) and map LLM_TIER_FAST_MODEL

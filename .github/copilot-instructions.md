...
- For Chinese text, use `老师` as the generic product term and `课程负责人` or `负责人`
  for a specific course owner; keep technical `creator*` identifiers unchanged
  unless a contract migration explicitly requires it.

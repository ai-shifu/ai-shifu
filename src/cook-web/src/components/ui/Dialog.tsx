...
import {
  composeRefs,
  useTriggerTransformOrigin,
} from '@/lib/dialog-transform-origin';
...
>(({ overlayClassName, ...contentProps }, ref) => {
  ...
  const originRef = useTriggerTransformOrigin();
  const contentRef = React.useMemo(
    () => composeRefs(originRef, ref),
    [originRef, ref],
  );
  ...
  return (
    <DialogPortal container={resolvedContainer}>
      ...
      <DialogPrimitive.Content
        ref={contentRef}
        className={cn(
          'fixed left-[50%] top-[50%] grid w-[calc(100vw-32px)] max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-90 data-[state=open]:zoom-in-90 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-1/2 data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-1/2 sm:w-full sm:rounded-lg',

...
const GotoPropsEqual = (prevProps: UIBlockDTO, nextProps: UIBlockDTO) => {
  ...
  for (let i = 0; i < prevGotoSettings.conditions.length; i++) {
    if (
      !_.isEqual(
        prevGotoSettings.conditions[i].value,
        nextGotoSettings.conditions[i].value,
      ) ||
      !_.isEqual(
        prevGotoSettings.conditions[i].destination_bid,
        nextGotoSettings.conditions[i].destination_bid,
      ) ||
      !_.isEqual(
        prevGotoSettings.conditions[i].destination_type,
        nextGotoSettings.conditions[i].destination_type,
      )
    ) {
      return false;
    }

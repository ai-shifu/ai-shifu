...
14. 当需要在 `ai-shifu` 本地稳定复现 `run` SSE 卡住问题时，优先在 `getRunMessage` 层增加显式调试开关，回放由 `测试数据.json` 截断得到的 stuck fixture，并保持 `message/readystatechange/close` 契约不变；不要把页面组件改成只兼容 mock 数据。

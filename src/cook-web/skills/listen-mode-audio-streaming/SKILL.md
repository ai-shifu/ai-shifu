...
16. 从阅读模式切到听课模式前，先扫描当前 lesson 已有正文中 `is_speakable !== false` 且有有效 `element_bid` 的内容块；缺音频时复用 `generated-blocks/:id/tts?listen=true` 补生成，并对同一 block 做 in-flight 去重。没有可补正文，或任一必需补音频最终失败时，才提示用户重修/继续阅读。

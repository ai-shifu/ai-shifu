...
- 对触发弹窗的入口按钮补轻量防抖或 single-flight 保护，避免双击导致多次打开弹窗或重复触发同一请求。
...
3. 若用户可能连续点击确认按钮，在组件层补 single-flight 包装，确保首个请求未完成前后续点击直接返回。
4. 对触发弹窗的入口按钮增加短时间防抖，避免双击造成重复埋点、重复打开或事件冒泡副作用。

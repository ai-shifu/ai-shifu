...
4. 使用课程 store 和当前 lesson 标题组装横屏 header 内容，保证无数据时仍可安全退化。
5. 把播放器设置文案补到 `chat` i18n 命名空间，并同步更新生成型的 key 类型文件。
6. 变更后至少执行一次定向类型检查，确认 module augmentation 和新 props 没有引入类型回归。

...
def test_unread_data_detected(sock_pair: tuple[socket.socket, socket.socket]) -> None:
...
def test_pool_discards_connection_desynced_during_use(
    sock_pair: tuple[socket.socket, socket.socket],
) -> None:
    left, right = sock_pair
...
def test_probe_timeout_waits_for_in_flight_data(
    sock_pair: tuple[socket.socket, socket.socket],
) -> None:

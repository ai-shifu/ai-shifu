...
def test_scope_token_is_stable_within_a_context(app) -> None:
...
def test_scope_token_never_repeats_across_contexts(app) -> None:
    tokens = []
    for _ in range(50):
        with app.app_context():
...
def test_nested_contexts_get_distinct_tokens(app) -> None:
    with app.app_context():
...
def test_sessions_are_isolated_per_context_and_removed_on_teardown(app) -> None:
    with app.app_context():

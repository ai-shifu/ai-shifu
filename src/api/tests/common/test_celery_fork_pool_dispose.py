...
def test_dispose_replaces_the_inherited_pool(app) -> None:
...
def test_worker_process_init_signal_disposes_pools(app) -> None:
    get_celery_app(app)
    with app.app_context():
        db.session.execute(text("SELECT 1"))
        db.session.remove()
...
def test_one_failing_bind_does_not_block_the_rest(app, monkeypatch) -> None:

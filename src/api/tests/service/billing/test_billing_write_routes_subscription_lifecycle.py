...
class TestBillingWriteRoutesSubscriptionLifecycle:
    ...
    def test_subscription_checkout_recreates_pending_stripe_order_when_callback_origin_changes(
        ...
        assert billing_write_client["stripe_expire_requests"] == ["cs_billing_test"]

    def test_subscription_checkout_refuses_callback_refresh_when_stripe_session_is_unsafe(
        self, billing_write_client, monkeypatch
    ) -> None:
        client = billing_write_client["client"]
        app = billing_write_client["app"]
        ...
        billing_write_client["stripe_session_statuses"]["cs_billing_test"] = {
            "id": "cs_billing_test",
        ...
        assert second_checkout["code"] == ERROR_CODE["server.order.orderStatusError"]
        assert len(billing_write_client["stripe_requests"]) == 1

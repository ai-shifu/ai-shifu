...
def test_listen_element_adapter_emits_final_fallback_text_snapshot(app):
    _require_app(app)
    ...
    user_bid = "user-final-fallback-text"
    shifu_bid = "shifu-final-fallback-text"
    outline_bid = "outline-final-fallback-text"
    progress_bid = "progress-final-fallback-text"
    generated_block_bid = "generated-final-fallback-text"
    ...
    with app.app_context():
        ...
        block = LearnGeneratedBlock(
            ...
            progress_record_bid=progress_bid,
            user_bid=user_bid,
            block_bid="block-final-fallback-text",
            outline_item_bid=outline_bid,
            shifu_bid=shifu_bid,
            type=0,
            role=ROLE_TEACHER,
        ...
        assert [item.type for item in streamed] == ["element", "element", "done"]
        assert len(element_events) == 2
        assert all(item.element_type == ElementType.TEXT for item in element_events)
        assert element_events[0].is_final is False
        assert element_events[1].is_final is True
        assert element_events[0].element_bid == element_events[1].element_bid
        assert element_events[1].content_text == "Fallback narration\n"
        ...
        assert persisted_row.is_final == 1
        assert persisted_row.content_text == "Fallback narration\n"

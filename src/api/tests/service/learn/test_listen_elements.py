...
def test_listen_adapter_patch_live_updates_keeps_single_active_fallback_row(app):
    _require_app(app)
    ...
    user_bid = "user-read-patch-fallback"
    shifu_bid = "shifu-read-patch-fallback"
    outline_bid = "outline-read-patch-fallback"
    generated_block_bid = "generated-read-patch-fallback"
    ...
    with app.app_context():
        ...
        assert element_events[0].is_new is True
        assert element_events[0].target_element_bid in ("", None)
        assert element_events[1].is_new is False
        assert element_events[1].target_element_bid == element_events[0].element_bid
        assert element_events[1].content_text == "Hello world"
        ...
        assert active_rows[0].is_new == 0
        assert active_rows[0].target_element_bid == element_events[0].element_bid
        assert active_rows[0].is_final == 1
        assert active_rows[0].content_text == "Hello world"

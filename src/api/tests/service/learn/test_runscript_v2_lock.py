...
def test_run_script_non_listen_patches_repeated_fallback_text(app, monkeypatch):
    with app.app_context():
        app.config["REDIS_KEY_PREFIX"] = "test"
        monkeypatch.setitem(app.config, "SSE_HEARTBEAT_INTERVAL", 0)
        lock = FakeLock([True])
        ...
        events = _parse_sse_events(chunks)
        element_events = [event for event in events if event["type"] == "element"]

        assert lock.acquire_calls == 1
        assert lock.release_calls == 1
        assert len(element_events) >= 2
        assert element_events[0]["content"]["is_new"] is True
        assert element_events[0]["content"].get("target_element_bid", "") == ""
        assert element_events[1]["content"]["is_new"] is False
        assert (
            element_events[1]["content"].get("target_element_bid")
            == element_events[0]["content"]["element_bid"]
        )
        assert element_events[1]["content"]["content"] == "Hello world"
        assert events[-1]["type"] == "done"

...
def _shared_callback(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
...
@pytest.fixture
def _verified_custom_domain(monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestIsAllowedOAuthOrigin:
    ...
    @pytest.mark.usefixtures("_verified_custom_domain")
    def test_http_custom_domain_is_refused(self, app: object) -> None:
    ...
    def test_lookup_failure_refuses_rather_than_allows(
        ...
        def _boom(app: object, host: object) -> Never:
            _ = (app, host)
    ...
    def test_malformed_origins_are_refused(self, app: object, value: object) -> None:
        assert oauth_origins.is_allowed_oauth_origin(app, value) is False

    @pytest.mark.usefixtures("_verified_custom_domain")
    def test_path_and_query_are_stripped(self, app: object) -> None:
        assert (
...
class TestPortAndUserinfoAreNotServedDomains:
    ...
    @pytest.mark.usefixtures("_verified_custom_domain")
    def test_userinfo_on_a_verified_domain_is_refused(self, app: object) -> None:
        assert (
    ...
    @pytest.mark.usefixtures("_verified_custom_domain")
    def test_explicit_default_port_still_resolves(self, app: object) -> None:
        assert (
...
class TestResolveStateReturnOrigin:
    ...
    @pytest.mark.usefixtures("_verified_custom_domain")
    def test_returns_the_recorded_origin(self, app: object) -> None:
    ...
    def test_state_without_origin_returns_empty(self, app: object) -> None:
        state = _encode_state(app, {"login_context": "default"})
        assert resolve_state_return_origin(app, state) == ""

    def test_tampered_state_returns_empty(self, app: object) -> None:
        assert resolve_state_return_origin(app, "not-a-valid-jwt") == ""

    def test_missing_state_returns_empty(self, app: object) -> None:
...
class TestInitiatorPairing:
    ...
    def test_another_session_cannot_complete_the_flow(self, app: object) -> None:
    ...
    def test_an_origin_without_an_initiator_is_refused(self, app: object) -> None:

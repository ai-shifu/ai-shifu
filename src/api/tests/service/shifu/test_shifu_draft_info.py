...
def test_alias_save_uses_original_fields_and_supports_live_roundtrip(
    ...
    with app.app_context():
        ...
        result = save(shifu_model="ultimate", ask_model="fast")
        assert result.model == "ultimate"
        assert result.ask_model == "fast"
        row = (
        ...
        first_id = row.id
        assert save().model == "ultimate"
        assert save(shifu_model="other-legacy").model == "other-legacy"
        result = save(ask_model=GEMINI_LIVE_MODEL_ID)
        assert result.ask_model == GEMINI_LIVE_MODEL_ID
        assert result.follow_up_mode == "live_voice"
        result = save(ask_model="balanced")
        assert result.follow_up_mode == "text"
        assert result.ask_model == "balanced"
        assert result.model == "other-legacy"
        assert db_latest_id("tier-save-course") > first_id
        # An explicit default selection stores Fast in the existing fields.

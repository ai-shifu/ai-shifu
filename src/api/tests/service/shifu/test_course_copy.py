...
def _seed_course_with_outlines(app, *, shifu_bid: str, creator_user_bid: str) -> dict[str, str]:
    draft = DraftShifu(
        shifu_bid=shifu_bid,
        title=SOURCE_TITLE,
...
def test_copy_course_allows_same_creator_and_clones_latest_draft(app):
    ...
    with app.app_context():
        ...
        assert copied_draft.title == f"{SOURCE_TITLE}{_('server.shifu.copyCourseTitleSuffix')}"
        ...
        assert copied_by_title['第一节'].parent_bid == copied_by_title['第一章'].outline_item_bid
        assert copied_by_title['第二节'].parent_bid == copied_by_title['第一章'].outline_item_bid
        assert copied_by_title['第二节'].prerequisite_item_bids == copied_by_title['第一节'].outline_item_bid
        ...
        assert copied_history.children[0].bid == copied_by_title['第一章'].outline_item_bid
        assert len(copied_history.children[0].children) == 2
        assert copied_auths == []


def test_copy_course_creates_missing_target_user_and_grants_creator_role(app, monkeypatch):
    shifu_bid = uuid.uuid4().hex[:32]
    creator_bid = uuid.uuid4().hex[:32]
    ...
    with app.app_context():
        ...
        _seed_course_with_outlines(app, shifu_bid=shifu_bid, creator_user_bid=creator_bid)
        db.session.commit()

...
- Write mainly in the learner's language, preserve natural mixed-language terms, keep facts in the first person, and organize present categories with short labels. Replace the source paragraph; never copy or quote it into the result.
- Explain background and experience enough to guide relevant examples and terminology; explain goals, concerns, and constraints enough to guide emphasis; turn language-style preferences into observable expression preferences. Only when the learner names a style, use high-level traits and prohibit imitation or reproduction; never add that boundary otherwise.

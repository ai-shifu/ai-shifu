...
def text_spilt(text: str, split_separator: str, split_max_length: int, split_chunk_overlap: int):
...
def get_vector_list(text_list: list, embedding_model: str):
    return [x.embedding for x in embedding_client.embeddings.create(
...
def kb_file_upload(
    ...
    with app.app_context():
        ...
        # text_spilt
        all_text_list = text_spilt(all_text, split_separator, split_max_length, split_chunk_overlap)
        number = 0
        processing_batch_size = 32
        for text_list in [all_text_list[i:i + processing_batch_size] for i in
                          range(0, len(all_text_list), processing_batch_size)]:
...
def retrieval_fun(kb_id: str, query: str, embedding_model: Optional[str] = None):
    embedding_model = get_embedding_model(kb_id, embedding_model)
    return '\n\n'.join([x['entity']['text'] for x in milvus_client.search(
        collection_name=kb_id,
        anns_field="vector",
        data=[get_vector_list(text_list=[query], embedding_model=embedding_model)[0]],
        limit=3,
...
def retrieval(app: Flask, kb_id: str, query: str, embedding_model: Optional[str] = None):
    with app.app_context():

...
def _parse_shifu_bids(raw: object) -> frozenset[str]:
    ...
    before that, so both are accepted. Blank entries are dropped rather than kept as an empty
    identifier that would match a course with no bid.
    """
    if raw is None:
        return frozenset()
    if isinstance(raw, str):
        candidates: Iterable[object] = raw.split(",")

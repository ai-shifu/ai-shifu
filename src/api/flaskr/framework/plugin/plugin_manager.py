...
class PluginManager:
    ...
    def enable_hot_reload(self: object) -> None:
    ...
    def disable_hot_reload(self: object) -> None:
        """Disable the hot reload."""
        if self.hot_reloader:
            self.hot_reloader.stop()
            self.hot_reloader = None

    def clear_extension(self: object, target_func_name: object) -> None:
    ...
    def register_extension(
        self: object, target_func_name: object, func: object
    ) -> None:
    ...
    def register_extensible_generic(
        self: object, func_name: object, func: object
    ) -> None:
...
def extensible(func: Callable[P, R]) -> Callable[P, R]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if manager is None:
            return result
        return cast(
            "R", manager.execute_extensions(func.__name__, result, *args, **kwargs)
        )
...
def extensible_generic(
    func: Callable[P, Iterable[object] | None],
    ...
    @wraps(func)
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> Generator[object, None, None]:
        result = func(*args, **kwargs)
        if result:
            yield from result
        manager = get_plugin_manager()

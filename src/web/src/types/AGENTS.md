...
This file adds local guidance for `src/web/src/types`. Read the repository
...
- Use focused frontend verification around `src/web/src/types/` before
...
- `rg -n "types" src/web/src` to locate call sites before editing this
  subtree.
...
- `cd src/web && npm run test -- src/types/` for focused Jest coverage when
  this subtree already has or gains tests.

## Tests

- Add or update focused tests closest to `src/web/src/types/` for the happy

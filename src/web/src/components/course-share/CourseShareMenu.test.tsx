...
import { CourseShareMenu } from './CourseShareMenu';
...
describe.each([
...
] as const)('CourseShareMenu on %s', surface => {
  const props = { ...baseProps, surface };
  const actor = surface === 'teacher_header' ? 'teacher' : 'learner';
  const open = () => {
  ...
  test('copies complete course prompt and closes with a next-step toast and privacy-safe events', async () => {
    const view = render(<CourseShareMenu {...props} />);
    open();
    view.rerender(<CourseShareMenu {...props} />);
    ...
    expect(mockToast).toHaveBeenCalledWith({ description: zh.posterNextStep });
    expect(mockTrack.mock.calls).toEqual([
      [`${actor}_course_share_open`, { shifu_bid: 'course-1', surface }],
      [`${actor}_poster_prompt_copy`, { shifu_bid: 'course-1', surface }],
      [
        `${actor}_poster_prompt_result`,
        { shifu_bid: 'course-1', surface, outcome: 'success' },
      ],
    ]);
  });
  ...
  test('reopening refreshes the current course data', async () => {
    ...
    expect(
      mockTrack.mock.calls.filter(
        ([event]) => event === `${actor}_course_share_open`,
      ),
    ).toHaveLength(2);
  });
  ...
  test('outside interaction dismisses without copying', async () => {
    ...
    fireEvent.pointerDown(screen.getByRole('button', { name: zh.share }), {
      pointerType: 'mouse',
    });
    fireEvent.focusIn(screen.getByRole('button', { name: zh.share }));
    await closed();
    expect(copy).not.toHaveBeenCalled();
  });
  ...
  test('opens the same two choices from an icon', () => {
    render(
      <CourseShareMenu
        {...props}
        showLabel={false}
    ...
    expect(
      within(popup).getByRole('button', { name: zh.posterCopy }),
    ).toBeVisible();
  });
});
